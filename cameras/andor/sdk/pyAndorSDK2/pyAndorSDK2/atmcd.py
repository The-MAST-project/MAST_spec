...
class atmcd:
    ...
    def _load_library(self, userPath) -> Union[WinDLL, CDLL]:
        if sys.platform == "linux":
            ...
            raise ATMCDException("Cannot detect operating system, will now stop")
    ...
    def GetHardwareVersion(self):
        ...
        ret = self.dll.GetHardwareVersion(
            byref(cPCB),
            byref(cDecode),
            byref(cdummy1),
            byref(cdummy2),
            byref(cCameraFirmwareVersion),
            byref(cCameraFirmwareBuild),
        )
        return (
            ret,
            cPCB.value,
            ...
            cdummy1.value,
            cdummy2.value,
            cCameraFirmwareVersion.value,
            cCameraFirmwareBuild.value,
        )
    ...
    def GetSoftwareVersion(self):
        ...
        cdllrev = c_uint()
        cdllver = c_uint()
        ret = self.dll.GetSoftwareVersion(
            byref(ceprom), byref(ccoffile), byref(cvxdrev), byref(cvxdver), byref(cdllrev), byref(cdllver)
        )
    ...
    def GetUSBDeviceDetails(self):
        ...
        ret = self.dll.GetUSBDeviceDetails(
            byref(cVendorID), byref(cProductID), byref(cFirmwareVersion), byref(cSpecificationNumber)
        )
    ...
    def PostProcessCountConvert(
        self, pInputImage, iOutputBufferSize, iNumImages, iBaseline, iMode, iEmGain, fQE, fSensitivity, iHeight, iWidth
    ):
        """
        Description:
        ...
        ret = self.dll.PostProcessCountConvert(
            byref(cpInputImage),
            byref(cpOutputImage),
            ciOutputBufferSize,
            ciNumImages,
    ...
    def PostProcessDataAveraging(
        ...
        ret = self.dll.PostProcessDataAveraging(
            byref(cpInputImage),
            byref(cpOutputImage),
            ciOutputBufferSize,
            ciNumImages,
            ciAveragingFilterMode,
            ciHeight,
            ciWidth,
            ciFrameCount,
            ciAveragingFactor,
    ...
    def PostProcessNoiseFilter(self, pInputImage, iOutputBufferSize, iBaseline, iMode, fThreshold, iHeight, iWidth):
        ...
        ciHeight = c_int(iHeight)
        ciWidth = c_int(iWidth)
        ret = self.dll.PostProcessNoiseFilter(
            byref(cpInputImage), byref(cpOutputImage), ciOutputBufferSize, ciBaseline, ciMode, cfThreshold, ciHeight, ciWidth
        )
        return (ret, cpOutputImage.value)

    def PostProcessPhotonCounting(
        self, pInputImage, iOutputBufferSize, iNumImages, iNumframes, iNumberOfThresholds, iHeight, iWidth
    ):
        """
        Description:
        ...
        ciHeight = c_int(iHeight)
        ciWidth = c_int(iWidth)
        ret = self.dll.PostProcessPhotonCounting(
            byref(cpInputImage),
            byref(cpOutputImage),
            ciOutputBufferSize,
            ciNumImages,
            ciNumframes,
            ciNumberOfThresholds,
            byref(cpfThreshold),
            ciHeight,
            ciWidth,
        )
    ...
    def SaveAsCalibratedSif(self, path, data_type, unit, coeff, rayleighWave):
        ...
        ret = self.dll.SaveAsCalibratedSif(cpath, cdata_type, cunit, ccoeff(*coeff), crayleighWave)
        return ret
    ...
    def SetMultiTrackScan(
        ...
        ret = self.dll.SetMultiTrackScan(
            ctrackHeight,
            cnumberTracks,
            ciSIHStart,
            ciSIHEnd,
