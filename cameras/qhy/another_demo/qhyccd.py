...
class QHYCCDSDK:
    ...
    def get_parameter_limits(self, camera_handle, parameter):
        ...
        self._sdk.GetQHYCCDParamMinMaxStep(
            camera_handle,
            parameter,
            ctypes.byref(param_min),
            ctypes.byref(param_max),
            ctypes.byref(param_step),
        )
    ...
    def version(self):
        ...
        ret = self._sdk.GetQHYCCDSDKVersion(
            ctypes.byref(year),
            ctypes.byref(month),
            ctypes.byref(day),
            ctypes.byref(subday),
...
class QHYCCDCamera:
    ...
    def __init__(self, sdk, camera_id, new_bpp=16):
        ...
        self.target_temperaturetarget_temperaturetarget_temperature = 10.0

        # Set ROI and readout parameters
        self._roi_w, self._roi_h = (
            ctypes.c_uint(self._width),
    ...
    def target_temperature(self, new_temperature):
        ...
        self._sdk.set_parameter(
            self._camera,
            CONTROL_ID.CONTROL_COOLER,
            ctypes.c_double(self._target_temperature),
        )
    ...
    def exposure_time(self, new_exposure_time):
        ...
        self._sdk.set_parameter(
            ...
            ctypes.c_double(self._exposure_time * 1e6),
        )
        log.debug(
            "Set exposure time to",
            self._sdk.get_parameter(self._camera, CONTROL_ID.CONTROL_EXPOSURE) / 1e6,
        )
    ...
    def set_roi(self, x0, y0, roi_w, roi_h):
        ...
        if self._bpp.value == 16:
            ...
            self._sdk._sdk.SetQHYCCDResolution(
                self._camera,
                ctypes.c_uint(x0),
                ctypes.c_uint(y0),
                self._roi_w,
    ...
    def readout(self):
        ret = self._sdk._sdk.GetQHYCCDSingleFrame(
            self._camera,
            ctypes.byref(self._roi_w),
            ctypes.byref(self._roi_h),
...
if __name__ == "__main__":
    ...
    cam_id = sdk.list_cameras()[0]
    cam = QHYCCDCamera(sdk, camera_id=0)
